...
universalize() {
    ...
    cp -RPc "$_mountpoint1" _universalized
    find "$_mountpoint1" -type f \( -perm +u+x -or -name '*.dylib' \) | while read -r _name1; do
        _relative_name="${_name1#"$_mountpoint1/"}"
        lipo "$_name1" "$_mountpoint2/$_relative_name" -create -output "_universalized/$_relative_name"
    done
    hdiutil detach "$_device1"
    hdiutil detach "$_device2"
    cd _universalized
    codesign_app "SQLiteStudio.app"
    # shellcheck disable=SC2086
    pretty_dmg "SQLiteStudio.app" "universal_out" "$BACKGROUND_IMG" $BACKGROUND_RGB
    mv "universal_out.dmg" "../$(echo "$1" | sed -E 's/arm64|x86_64/universal/')"
    cd ..
    rm -fr _universalized
}

if [ "$universalize" -gt 0 ]; then
    universalize "$@"
    exit
fi
